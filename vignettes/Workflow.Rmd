...
For Area Of Habitat (AOH) maps and similar, we only want to retain habitats which are suitably suitable. `RL_subset_acceptable()` allows you to do this by specifying which suitabilities and major importances are good enough. Those which are not are recoded as `NA`.
...
```
See `?RL_crosswalk()` for more information.
